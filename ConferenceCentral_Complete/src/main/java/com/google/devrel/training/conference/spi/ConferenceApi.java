...
@Api(name = "conference", version = "v1", scopes = { Constants.EMAIL_SCOPE }, clientIds = { Constants.WEB_CLIENT_ID,
		Constants.ANDROID_CLIENT_ID, Constants.API_EXPLORER_CLIENT_ID }, audiences = {
				Constants.ANDROID_AUDIENCE }, description = "Conference Central API for creating and querying conferences,"
...
public class ConferenceApi {
	...
	private static Profile getProfileFromUser(User user, String userId) {
		...
		if (profile == null) {
			...
			profile = new Profile(userId, extractDefaultDisplayNameFromEmail(email), email, TeeShirtSize.NOT_SPECIFIED);
		}
		return profile;
	}
	 ...
	 *             when the user is not the owner of the Conference.
	 */
	...
	public Conference updateConference(final User user, final ConferenceForm conferenceForm,
		...
		TxResult<Conference> result = ofy().transact(new Work<TxResult<Conference>>() {
			...
			public TxResult<Conference> run() {
				...
				if (conference == null) {
					return new TxResult<>(
							new NotFoundException("No Conference found with the key: " + websafeConferenceKey));
				}
				// If the user is not the owner, throw a 403 error.
				Profile profile = ofy().load().key(Key.create(Profile.class, userId)).now();
				if (profile == null || !conference.getOrganizerUserId().equals(userId)) {
					return new TxResult<>(new ForbiddenException("Only the owner can update the conference."));
				}
	...
	public WrappedBoolean registerForConference(final User user,
		...
		TxResult<Boolean> result = ofy().transact(new Work<TxResult<Boolean>>() {
			...
			public TxResult<Boolean> run() {
				...
				if (conference == null) {
					return new TxResult<>(
							new NotFoundException("No Conference found with key: " + websafeConferenceKey));
				}
...
//        Conference conference = ofy().load().key(conferenceKey).now();
//        if(conference == null)
//        	throw new UnauthorizedException("No Conference fund with this key");
	...
	public Session createSession(final User user, final SessionForm sessionForm) throws Exception {
		if (user == null) {
...
//			throw new Exception("Conference is required");
		...
		Integer duration = sessionForm.getDuration();
		List<String> highlights = sessionForm.getHighlights();
		String name = sessionForm.getName() + " from conference " +conference.getId();
    ...
    public Session getSession(@Named("websafeSessionKey") final String websafeSessionKey) throws Exception{
